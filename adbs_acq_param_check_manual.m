function adbs_acq_param_check_manual(nifti_dir, dicom_dir, out_dir, dcm2niix_dir, in_file)
% Function to compile subject details and acquisition parametes (subj-ID,
% name, age, gender, TR, TE, image size, and voxel size) into a csv file
% based on list of subject IDs and filenames provided by the user
%% Inputs:
...
% in_file:      a text file containing the list of subjects and their
%               filenames which would be used for parameter check (i.e. we
%               search for these filenames in the NIfTI folder, get the
%               series number, and then go for parameter check); has to be
%               in the format: sub-ID <tab space> filename.nii (see Notes)
% 
%% Output:
% Given an input file with subject IDs and filenames, the series ID for
% that acquisition is picked for each subject from the .json files; then
% that particular DICOM data is converted to NIfTI while additionally
% writing out the text file containig subject's private data from which
% name, age, gender, TR and TE is picked up; the image is then read using
% SPM and image and voxel size is calculated.
%
% Following files are created in the out_dir apart from the NIfTI images:
% (param_check_manual_ddmmmyyyy_hhmmss.csv)
% (param_check_summary_manual_ddmmmyyyy.txt)
%
%% Notes:
% Only subject IDs mentioned in the in_file are processed; if any of the
% subject IDs exist in the out_dir, that subject is skipped; this is useful
% if the same nifti_dir and dicom_dir have to be repeatedily subjected to
% conversion (such as the case when new subjects are added) but existing
% data should not be touched (therefore, it is implied that the subject IDs
% mentioned in in_file are the same as the folder names in the nifti_dir
% and dicom_dir locations)
% 
% Folder corresponding to each subject is automatically created in the
% out_dir, the name is the same as the name in nifti_dir and dicom_dir
%
...
% being used
%
% If dcm2niix_dir is not provided, user is prompted via GUI to select the
% appropriate directory; this can lead to a crash if remote session is
% being used
% 
% If in_file is not provided, user is promped via GUI to select the
% appropriate file; this can lead to a crash if remote session is being
% used
%
...
%% Create subject list by parsing in_file
num_subjs  = length(to_read_info{2});
list_subjs = to_read_info{1};
disp([num2str(num_subjs), ' subjects found']);

%% Prepare summary file
fid_summary_name = fullfile(out_dir, ['param_check_summary_manual_', ...
...
for subj = 1:num_subjs
    ...
    % Output path
    sub_out_dir = fullfile(out_dir, list_subjs{subj});
    
    % If output directory exists, skip the subject
    if exist(sub_out_dir, 'dir')
        disp([list_subjs{subj}, '...skipped']);
